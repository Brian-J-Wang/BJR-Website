...
const LocationMap = () => {
	...
	useEffect(() => {
		...
		const manhattanContainer = document.createElement("div");

		createRoot(manhattanContainer).render(<MapMarker></MapMarker>);

		// Manhattan Office
		const manhattanMarker = new maplibregl.Marker({
			element: manhattanContainer,
			anchor: "left",
		})
			.setLngLat(manhattenCoords)
			.addTo(map.current);
		...
		// Brooklyn Office
		const brooklynMarker = new maplibregl.Marker({ color: "red" })
